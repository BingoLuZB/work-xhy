...
let fs_inCacheStage = true;
const re = '/(effect|zjm_effect|ui_effect|sound)/.+'

export const fs = {

    needCache: (url) => {
        if (window.fs_inCacheStage)
            return true
        else {
            return url.match(re) && true
        }
    },
...
window.fs_stats = fs_stats;
window.fs_inCacheStage = fs_inCacheStage;