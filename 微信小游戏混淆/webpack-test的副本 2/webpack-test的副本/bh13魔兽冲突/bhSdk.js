class MiniSdk {
  constructor() {
    ...
    wx.onShow((res) => {
      ...
      if (res.query.isAward) that.getShareAward(res.shareTicket, this.shareCallback)
    })
  }
      ...
      success: res => {
        ...
        wx.request({
          ...
          success: res => {
            ...
            // 判断用户是否需要实名验证
            cpUserInfo.delayTime = resData.auth_conf.delayTime
            if (resData.auth_conf.isAuth == false) { //不需要实名认证
              that.userInfo = resData;
              // 可否进入游戏
              if (resData.auth_conf.isEnterGame == false) { //不能进入游戏
                wx.showModal({
                  title: '提示',
              ...
              if (resData.auth_conf.delayTime === 0) { //不能进入游戏
          ...
          success: res => {
            ...
            if (res.data.code === 200) {
              ...
              if (data.partner_data.type == 2) {
                ...
                wx.request({
                  // url: 'https://play.kunyufun.com/api/v1/play/create',
 ...
 */
   ...
   */
  gotoService(cardMsg) {
    // this.goShare('', function() {console.log(321), 1})
