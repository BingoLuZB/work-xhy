class MiniSdk {
  constructor() {
    // 接口地址
    this.protocol = "https";
    this.apiPrefix = "https://api.kunyufun.com/api/v1";
    this.sdkPrefix = "https://sdkapi.kunyufun.com/api/v1";
    this.orderPrefix = "https://order.kunyufun.com/api/v1";
    this.playPrefix = "https://play.kunyufun.com/api/v1";
    this.logPrefix = "https://log.kunyufun.com/api/v1";
    this.partnerFix = "https://partner.kunyufun.com/api/v1";
    this.notifyrFix = "https://notify.kunyufun.com/api/v1";
    ...
    wx.onShow((res) => {
      const that = this
      that.isAward = res.query.isAward || 0
      if (res.query.isAward) that.getShareAward(res.shareTicket)
    })
  }
  ...
  setTag(obj) {
    ...
    wx.request({
      url: `${this.apiPrefix}/mini/show`, //正式
  ...
  init(config, callBack = "") {
    ...
    wx.request({
      ...
      success: res => {
        ...
        if (resData.code === 200) {
          const initMlh5 = {
            appId: parseInt('64039'),
            channelId: parseInt('640394802'),
            appKey: '43B18C607E15ACEE9E4290141AE43D24',
            version: "1.0.1",
            callback: () => {
              console.log('初始化成功')
  ...
  sendPayNotify() {
    ...
    if (wx.getStorageSync("accessToken") && wx.getStorageSync("orderNo")) {
      ...
      that.repay_empty_count++;
      wx.request({
        url: `${that.notifyrFix}/partner/call/${that.config.pid}/${
          that.config.gid
          }`,
        data: notifyData,
        method: "post",
  ...
  loginFun(loginCallBack) {
    GameGlobal.mlh5.login({
      callback: (res) => {
        console.log(res, '===res')
        console.log('登录')
        GameGlobal.wfuserId = res.userId
        GameGlobal.wfuserToken = res.userToken
        GameGlobal.wfsessionKey = res.extData.sessionKey
        GameGlobal.wfopenId = res.extData.openId
        GameGlobal.wfunionId = res.extData.unionId
        console.log(res)
        const that = this;
          ...
          success: res => {
            let loginData = {
              ...
              partner_login_data: JSON.stringify({
                ...
                system: that.system,
                client_id: that.client_id,
                userId: GameGlobal.wfuserId,
                userToken: GameGlobal.wfuserToken,
                session_key: GameGlobal.wfsessionKey,
                openid: GameGlobal.wfopenId,
                unionId: GameGlobal.wfunionId
              })
            ...
            wx.request({
              ...
              success: res => {
                ...
                that.config.p_mid = resData.p_mid;
                ...
                // 判断用户是否需要实名验证
                if (resData.auth_conf.isAuth == false) {
                  that.userInfo = resData;
                  // 可否进入游戏
                  if (resData.auth_conf.isEnterGame == false) {
                    wx.showModal({
                      title: '提示',
                      content: resData.auth_conf.msg,
                      showCancel: false,
                      success(res) {
                        if (res.confirm) {
                  ...
                  //此处需要cp调起实名认证窗口
                  console.log('请在这里调起实名认证窗口')
                  that.loginCallback(that.checkRes(400, cpUserInfo, "need certification"));
  ...
  getCertificationMessage(idMessage) {
    ...
    wx.request({
      url: 'https://sdkapi.kunyufun.com/api/v1/bindInfo/idCard',
      data: bhData,
      method: "POST",
      success: res => {
  ...
  nameCertification(idName, idNumber, callBack, flag = true) {
    ...
    wx.request({
      url: 'https://sdkapi.kunyufun.com/api/v1/verify/id',
      data: bhData,
      method: "POST",
      success: res => {
        if (res.data.code == 200) {
  ...
  statisticalTime(time) {
    ...
    let timer = setInterval(() => {
      ...
      bhData.sign = this.signData(bhData)
      wx.request({
        url: 'https://sdkapi.kunyufun.com/api/v1/sdk/user/heartbeat',
        data: bhData,
        method: "POST",
        success: res => {
  ...
  setLevelVersion(data) {
    ...
    bhData.sign = that.signData(bhData),
      ...
      wx.setUserCloudStorage({
        KVDataList: [{ key: "newslevel", value: wxData }],
        success: res => { //接口调用成功
          console.log(res);
          wx.request({
            url: 'https://partner.kunyufun.com/api/v1/partner/report',
            data: bhData,
            method: "POST",
            header: {
   ...
   * 游戏圈按钮
   */
  gameClub(icon = 'dark', style = { left: 10, top: 13, width: 27, height: 27 }) {
  ...
  checkContent(content, cb = null) {
    const that = this
    let data = {
      pid: that.config.pid | 1,
      gid: that.config.gid | 1,
      tm: Date.parse(new Date()) / 1000,
      partner_data: JSON.stringify({
        content
      })
      ...
      if (client_id) {
        ...
        wx.request({
          ...
          success: res => {
            ...
            if (res.data.code === 200) {
              let orderData = data.partner_data
              orderData.callback=(res)=>{
                console.log(res)
              }
              GameGlobal.mlh5.pay(orderData)
 ...
 */
  ...
  useServicePay() {
    const that = this;
    let cardMsg = {
      img: "https://h5.kunyufun.com/miniGameImg/servicePay.jpg"
    }
    wx.showModal({
      title: "支付提示",
  ...
  getCusward(roleInfo) {
    const that = this;
    let cardMsg = {
      img: "https://h5.kunyufun.com/miniGameImg/getcCusward.png"
    }
    wx.request({
