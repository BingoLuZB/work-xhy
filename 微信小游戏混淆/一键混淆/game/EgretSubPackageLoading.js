class EgretSubPackageLoading extends egret.DisplayObjectContainer {
    ...
    onAddToStage() {
      ...
      if (wx.getStorageSync('changeJudge')) {
        content = wx.getFileSystemManager().readFileSync("bg_dengluditu.jpg");
      } else {
        content = ''
      }
    ...
    setProgress(res) {
      if (!changeJudge) return
      // iOS真机为totalBytesWriten 微信官方将于近期修复 2018.6.19
      console.log("EgretSubPackageLoading", res.progress)
      console.log(res.progress);
      this.shape.graphics.lineTo(egret.MainContext.instance.stage.stageWidth / 4 * 1 + egret.MainContext.instance.stage.stageWidth / 2 * (res.totalBytesWritten || res.totalBytesWriten) / res.totalBytesExpectedToWrite, egret.MainContext.instance.stage.stageHeight / 8 * 7);
