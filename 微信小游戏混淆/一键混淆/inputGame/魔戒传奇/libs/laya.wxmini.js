...
var MiniAdpter=(function(){
	...
	MiniAdpter.init=function(isPosMsg,isSon){
		...
		//加载json,并缓存在LocalStorage;
		Loader.prototype._loadJson = function(url){
			var _this = this;
			var index = url.lastIndexOf('?');
			var sourceurl;
			var version;
			// console.log('loadstart',url,Laya.Browser.now())
			if(index>=0)
			{
				sourceurl = url.substr(0,index);
				version = url.substr(index+1);
			}
			else
			{
				sourceurl = url;
			...
			// var cachedata=LocalStorage.getJSON(sourceurl);
			// if(cachedata)
			// {
			// 	if(cachedata.v == version)
			// 	{
			// 		Laya.timer.once(1,_this,function(){
			// 			_this.onLoaded(cachedata.data);
			// 		});
			// 		return;
			// 	}
			// }
			wx.downloadFile(
				{
					...
					success:function(data){
						console.log('loadjson',url,data.tempFilePath);
						MiniFileMgr.fs.readFile({
							...
							success: function(data) {
								try
								{
									...
									// LocalStorage.setJSON(sourceurl,{v:version,data:data})
								}
								catch(e)
								{
									console.log('jsonparseerror',url)
									_this.onError(e);
									return;
								}
								_this.onLoaded(data);
	...
	MiniAdpter.minClearSize=(10 *1024 *1024);
...
var MiniFileMgr=(function(){
	...
	MiniFileMgr.cacheReg = new RegExp('res/atlas/res');
	MiniFileMgr.checkCacheUrl=function(url){
		return true;
		// return MiniFileMgr.cacheReg.test(url);
	}
	...
	MiniFileMgr.readFile=function(filePath,encoding,callBack,readyUrl,isSaveFile,fileType,isAutoClear){
		...
		MiniFileMgr.fs.readFile({filePath:filePath,encoding:encoding,success:function (data){
				if (filePath.indexOf("http://")!=-1 || filePath.indexOf("https://")!=-1){
					if(MiniFileMgr.checkCacheUrl(readyUrl) || isSaveFile){
	...
	MiniFileMgr.tempFiles ={};
	MiniFileMgr.downOtherFiles=function(fileUrl,callBack,readyUrl,isSaveFile,isAutoClear){
		(readyUrl===void 0)&& (readyUrl="");
		(isSaveFile===void 0)&& (isSaveFile=false);
		(isAutoClear===void 0)&& (isAutoClear=true);
		if(MiniFileMgr.tempFiles[fileUrl])
		{
			callBack.runWith([0,MiniFileMgr.tempFiles[fileUrl]]);
			return;
		}
		MiniFileMgr.wxdown(
			{
				...
				success:function (data)
					{
						if (data.statusCode===200){
								MiniFileMgr.tempFiles[fileUrl]=data.tempFilePath;
								callBack.runWith([0,data.tempFilePath]);
								// if((MiniFileMgr.checkCacheUrl(readyUrl) || isSaveFile)&& readyUrl.indexOf("wx.qlogo.cn")==-1 && readyUrl.indexOf(".php")==-1)
		...
		if (fileObj){
			...
			MiniFileMgr.fs.getFileInfo({
				...
				success:function (data){
					...
					callBack !=null && callBack.runWith([0,tempFilePath]);
					MiniFileMgr.fs.copyFile({srcPath:tempFilePath,destPath:saveFilePath,success:function (data2){
							MiniFileMgr.onSaveFile(readyUrl,tempFileName,true,encoding,null,data.size);
							},fail:function (data){
								// callBack !=null && callBack.runWith([1,data]);
							// callBack !=null && callBack.runWith([0,tempFilePath]);
					}});
...
var MiniImage=(function(){
	...
	MiniImage.onCreateImage=function(sourceUrl,thisLoader,isLocal,tempFilePath){
		...
		(tempFilePath===void 0)&& (tempFilePath="");
		var fileNativeUrl;
		if(MiniFileMgr.checkCacheUrl(sourceUrl)){
			if (!isLocal){
				if(tempFilePath !=""){
		...
		var onerror=function (){
			clear();
			MiniFileMgr.onSaveFile(sourceUrl,null,false,null,null,null);
			htmlImage.destroy();
		...
		if (thisLoader._type=="nativeimage"){
			...
			thisLoader.imgCache[sourceUrl]=image;
				...
				htmlImage=new HTMLImage.create(fileNativeUrl,{onload:onload,onerror:onerror,onCreate:function (img){
					image=img;
...
var MiniLoader=(function(_super){
	...
	*/
	__proto.load=function(url,type,cache,group,ignoreCache){
		...
		// if(MiniFileMgr.checkCacheUrl(url))
		// {
		// 	console.log(url);
		// }
		var uurl = URL.formatURL(url);
		if (!ignoreCache && Loader.loadedMap[uurl]){
			thisLoader._data=Loader.loadedMap[uurl];
		...
		if ((MiniLoader._fileTypeArr.indexOf(urlType)!=-1)){
			...
			var fileObj = MiniFileMgr.getFileInfo(uurl);
			if (!fileObj){
				if (MiniFileMgr.isLocalNativeFile(url)){
					if (MiniAdpter.subNativeFiles && MiniAdpter.subNativeheads.length==0){
						for (var key in MiniAdpter.subNativeFiles){
...
var MiniSound=(function(_super){
	...
	*/
	__proto.load=function(url){
		...
		if(this.iscache&&MiniFileMgr.getFileInfo(url)){
	...
	*/
	__proto.play=function(startTime,loops){
		...
		if(this.iscache&&MiniFileMgr.getFileInfo(this.url)){
			var fileNativeUrl;
